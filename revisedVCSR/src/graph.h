...
class CSRGraph {
 ...
 public:
  ...
  CSRGraph(EdgeList &edge_list, bool is_directed, int64_t n_edges, int64_t n_vertices) {
    ...
    edges_=(DestID_ *)aligned_alloc(4096,elem_capacity*sizeof(DestID_));
    ...
    for (int i = 0; i < num_edges_; i++)
    {
      ...
      if (t_degree == 0) vertices_[t_src].index = i;  //define the offset of each vertex
...
//    int64_t *new_indices = calculate_positions(0, num_vertices, gaps);
    ...
    printf("this recount time spends %f seconds\n",(end.tv_sec-mid.tv_sec)+(double)(end.tv_usec-mid.tv_usec)/1000000);
    printf("this double time spends %f seconds\n",(end.tv_sec-start.tv_sec)+(double)(end.tv_usec-start.tv_usec)/1000000);
  } */
...
//    int64_t *new_indices = calculate_positions(0, num_vertices, gaps);
    ...
    DestID_ *new_edges_ = (DestID_ *) aligned_alloc (4096,sizeof(DestID_) * elem_capacity);
    int num_pages=elem_capacity/256;
    int double_granularity=256;
    void * new_addr=(void*) new_edges_;
    void * old_addr=(void*) edges_; 
    for (int i = 0; i < num_pages/(2*double_granularity);i++) {
      ...
      DestID_ * old_addr1=(DestID_ *)old_addr;
      DestID_ * new_addr1=(DestID_ *)new_addr;
      old_addr1+=256*double_granularity;
      new_addr1+=512*double_granularity;
      old_addr=(void*)old_addr1;
      new_addr=(void*)new_addr1;
    } 
    gettimeofday(&mid,NULL);
    //here start modify offset
    auto new_vertices_ = (struct vertex_element *) calloc (num_vertices, sizeof(struct vertex_element));
    ...
    for(int i=0;i<num_pages/(2*double_granularity);i++){
      int end_index_edge=256*(i+1)*double_granularity;
      if(i==0){
        for(int j=0;j<num_vertices;j++){
          if(vertices_[j].index<end_index_edge && vertices_[j+1].index>=end_index_edge){
            node_indexk=j;
            break;
          }
        }
        for(int j=node_indexm;j<=node_indexk;j++){
          new_vertices_[j].index=vertices_[j].index+i*256*double_granularity;
          new_vertices_[j].degree=vertices_[j].degree;
        }
      ...
      int flag_v=0;
      if(i!=num_pages/(2*double_granularity)-1){
        for(int j=slide_v;j<num_vertices;j++){
          if(vertices_[j].index<end_index_edge && vertices_[j+1].index>=end_index_edge){
            flag_v=j;
            break;
          }
        } 
      }else{flag_v=num_vertices-1;}
      
      if(flag_v==node_indexk){
        continue;
      }else{
        node_indexk=flag_v;
        for(int j=node_indexm;j<=node_indexk;j++){
    ...
    recount_segment_total();
    gettimeofday(&end,NULL);
    double_time+=(end.tv_sec-start.tv_sec)+(double)(end.tv_usec-start.tv_usec)/1000000;
    //printf("this midufy time spends %f seconds\n",(end.tv_sec-mid.tv_sec)+(double)(end.tv_usec-mid.tv_usec)/1000000);
    //printf("the ele caacity is %d\n",elem_capacity);
    //printf("this double time spends %f seconds\n",(end.tv_sec-start.tv_sec)+(double)(end.tv_usec-start.tv_usec)/1000000);
  } 
...
//    int64_t *new_indices = calculate_positions(0, num_vertices, gaps);
    ...
    for(int i=0;i<num_pages/(2*double_granularity);i++){
      int end_index_edge=256*(i+1)*double_granularity;
      if(i==num_pages/(2*double_granularity)-1){
        printf("the last vertex offset is %d\n",vertices_[num_vertices-1].index);
        printf("here!!\n");
        printf("the num vertices is %d\n",num_vertices);
        printf("the m  is %d\n",node_indexm);
        node_indexk=num_vertices-1;
        for(int j=node_indexm;j<=node_indexk;j++){
          vertices_[j].index+=i*256*double_granularity;
        }
        break;
      }else{
        for(int j=0;j<num_vertices;j++){
      ...
      if(i==0){
        node_indexm=node_indexk+1;
        continue;  //offsets are not changed for the first pages
      }
      else if (vertices_[node_indexk].index>=end_index_edge-256*double_granularity){
        for(int j=node_indexm;j<=node_indexk;j++){
          vertices_[j].index+=i*256*double_granularity;
        }
        node_indexm=node_indexk+1;
  ...
  void compute_capacity()
  {
    ...
    int elem_capa_remap=0;
    elem_capa_remap=hyperfloor(num_edges_);
    elem_capacity = elem_capa_remap * max_sparseness;
...
//    int64_t * new_positions = calculate_positions(start_vertex, end_vertex, gaps);
    ...
    for(int32_t curr_vertex=end_vertex-1; curr_vertex>start_vertex; curr_vertex-=1) {
      ...
      {
        edges_[write_index] = edges_[read_index];
        edges_[read_index]=0;  //need to set 0?
    ...
    recount_segment_total();   //compute the upper bound of the volume of each vertex segment
