...
//  }
  ...
  CSRGraph<NodeID_, DestID_, invert> MakeGraph() {
    ...
    srand(89750);
    random_shuffle(el.begin(), el.end());
    EdgeList base_el(el.begin(),el.begin()+(int)el.size()/10);

    std::sort(base_el.begin(), base_el.end(), [](Edge &a, Edge &b) {
      if(a.u != b.u) return a.u < b.u;
      return (a.v.v < b.v.v);
    });

    EdgeList dynamic_el(el.begin()+(int)el.size()/10,el.end());

    //build the first 10% graph
    CSRGraph<NodeID_, DestID_, invert> g(base_el, !symmetrize_, base_el.size(), num_nodes_);
    //build the last 90% graph
    t.Start();
    for(uint32_t i=0; i<dynamic_el.size(); i+=1) {
      //printf("the edge is %d\n",i);
      g.insert(dynamic_el[i].u, dynamic_el[i].v.v, dynamic_el[i].v.w);
      if(symmetrize_) {
...
//      if(i && i % 10000000 == 0) cout << "inserted " << (i/1000000) << "M dynamic edges" << endl;
    ...
    cout << "----Double Time: " << double_time << " seconds.----" << endl;
    exit(-1);
